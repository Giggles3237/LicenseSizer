...
export default function SupportPage() {
  return <LegalShell eyebrow="Help center" title="How can we help?" intro="Choose the contact that matches your question. Never send a license image, PDF, license number, or other identity document to a LicenseResizer support address.">
    ...
    <section><h2>Customer sharing help</h2><ol><li>Create and review the PDF in LicenseResizer.</li><li>Use the share sheet if your device supports PDF file sharing, or download the PDF.</li><li>Select the intended email or messaging application and verify the dealership recipient.</li><li>Confirm the PDF is attached, then send it from that application.</li></ol><p>LicenseResizer cannot see the selected app or recipient and cannot confirm delivery. Contact the dealership directly if you need confirmation.</p><p>Email and text draft links cannot reliably attach local PDF files from a browser. Use the share sheet when available, or download the PDF and attach it yourself.</p></section>
    <section><h2>Printing the PDF</h2><p>If a dealership asks you to print the file, open the PDF and choose Actual size or 100%. Turn off Fit, Scale to page, Shrink oversized pages, or similar print scaling options.</p></section>
  </LegalShell>;
}
